...
local crc16_ccitt_table_reverse = {
...
local function crc16_unreflected(data, seed, table)
	local crc = seed or 0xFFFF
	local j

	for i = 1, #data do
		j = band(bxor(rshift(crc, 8), byte(sub(data, i, i))), 0xFF)
		crc = bxor(table[j + 1], lshift(crc, 8))
	end

	return crc
end

local function crc16_reflected(data, seed, table)
	local crc = seed or 0xFFFF
	local j

	for i = 1, #data do
		j = band(bxor(crc, byte(sub(data, i, i))), 0xFF)
		crc = bxor(table[j + 1], rshift(crc, 8))
	end

	return crc
end

local function crc16(data, seed)
	return crc16_unreflected(data, seed, crc16_ccitt_table)
end

print(string.format("%x", crc16("asdasd")))
